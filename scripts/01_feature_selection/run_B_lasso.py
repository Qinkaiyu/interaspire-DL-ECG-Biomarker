#!/usr/bin/env python3
"""Step B: LASSO (L1) feature selection for ECG biomarkers."""
...
M3_VARS = [
    "CVARAGE","RQSEX","CVARCOSMOKING","CVARBMI","CVARSYSTOL",
    "CVARLABHBA1CPERCENT","CVAREGFR","CVARLABHDL","CVARLABLDLFRIEDEWALD",
    "RQHISTOFPRECVD","RQHISTOFCORARTDIS","RQHISTOFHEARTFAIL","any_af",
...
DISPLAY_ECG = {
    "Atrial Fibrillation":"AF (ECG)","LBBB":"LBBB","RBBB":"RBBB",
...
def bins(s, e): return pd.cut(s, bins=e, labels=range(len(e)-1), include_lowest=True).astype(float)
...
for col, edges in BIN_EDGES.items():
    if col in df.columns: df[col] = bins(df[col], edges)
...
all_vars   = M3_VARS + ECG_ALL
avail      = [v for v in all_vars if v in df_lr.columns]
X_raw      = df_lr[avail]
feat_names = avail

imp = SimpleImputer(strategy="median")
sc  = StandardScaler()
Xf  = sc.fit_transform(imp.fit_transform(X_raw))

# ── LASSO CV (single-threaded) ────────────────────────────────────────────────
log("Fitting LASSO (L1) with 5-fold inner CV …")
...
lasso_cv.fit(Xf, y)
log(f"  Best C: {lasso_cv.C_[0]:.5f}")

coefs = pd.Series(lasso_cv.coef_[0], index=feat_names)
ecg_coefs = coefs[[v for v in ECG_ALL if v in feat_names]]
...
for v in ECG_ALL:
    ...
    c = coefs[v]
    rows.append({
        "Variable"  : DISPLAY_ECG.get(v, v),
        "raw_col"   : v,
        "LASSO coef": round(c, 5),
        "Selected"  : c != 0,
    })
    log(f"  {DISPLAY_ECG.get(v,v):30s}  coef={c:.5f}  {'← selected' if c!=0 else ''}")

out_df = pd.DataFrame(rows).sort_values("LASSO coef", key=abs, ascending=False)
out_df.to_csv(OUT_DIR / "lasso_ecg_coefs.csv", index=False)

selected = [r["raw_col"] for r in rows if r["Selected"]]
log(f"\nSelected {len(selected)} ECG features: {[DISPLAY_ECG.get(v,v) for v in selected]}")
log("Saved → results/feature_selection/lasso_ecg_coefs.csv")
log("✓ Step B done.")
