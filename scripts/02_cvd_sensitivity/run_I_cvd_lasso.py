#!/usr/bin/env python3
"""Step I: LASSO feature selection — CVD composite endpoint (sensitivity analysis)."""
...
lasso_cv=LogisticRegressionCV(Cs=Cs,cv=5,penalty="l1",solver="liblinear",
...
for v in ECG_ALL:
    ...
    rows.append({"Variable":DISPLAY_ECG.get(v,v),"raw_col":v,
                 "LASSO coef":round(c,5),"Selected":c!=0})
...
out.to_csv(OUT_DIR/"cvd_lasso_ecg_coefs.csv",index=False)
selected=[r["raw_col"] for r in rows if r["Selected"]]
log(f"\nSelected {len(selected)}: {[DISPLAY_ECG.get(v,v) for v in selected]}")
log(f"Best C saved for stability step: {BEST_C:.5f}")
(OUT_DIR/"cvd_best_c.txt").write_text(str(BEST_C))
log("Saved → cvd_lasso_ecg_coefs.csv")
log("✓ Step I done.")
