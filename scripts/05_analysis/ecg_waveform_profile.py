...
Approach:
  ...
  2. Compute per-patient median RR interval
  3. Extract full single-beat: from -0.35*RR to +0.65*RR around each R-peak
  4. Resample each beat to a common length (normalized beat)
  5. Average across patients per risk group
  6. Use SEM (not SD) for narrower confidence bands
"""
...
# ── Config ──────────────────────────────────────────────────────────────────
ECG_DIR = Path("data/ecg_matrices/preprocessed")
M5_PATH = Path("model5_cvd_mace4/E2E_MultiTask_32_2_phase_cvd_mace4_1yr_oof.csv")
OUT_DIR = Path("results/figures/ecg_waveform_profiles")
...
def extract_rr_beats(ecg_12lead, r_peaks, sr=500):
    ...
    For beat i (R-peak at r_peaks[i]):
      - start = midpoint between r_peaks[i-1] and r_peaks[i]
      - end   = midpoint between r_peaks[i] and r_peaks[i+1]
    This gives each beat its actual duration based on surrounding RR intervals.
    Then resample to RESAMPLE_LEN common points.
    """
    if len(r_peaks) < 3:
        return None
    ...
    for i in range(1, len(r_peaks) - 1):
        # Beat boundaries at midpoints between consecutive R-peaks
        ...
        if beat_len < int(0.3 * sr) or beat_len > int(2.0 * sr):
            # Skip abnormally short (<300ms) or long (>2000ms) beats
            continue

        beat = ecg_12lead[:, start:end]  # (12, beat_len) — variable length
        # Resample to common length
        beat_resampled = resample(beat, RESAMPLE_LEN, axis=1)  # (12, RESAMPLE_LEN)
        beats.append(beat_resampled)

    if len(beats) < 2:
        return None
    return np.stack(beats)  # (n_beats, 12, RESAMPLE_LEN)
...
for grp in ["Low", "Intermediate", "High"]:
    arr = np.stack(group_beats[grp])  # (n_patients, 12, RESAMPLE_LEN)
    group_mean[grp] = arr.mean(axis=0)
    group_sem[grp] = arr.std(axis=0) / np.sqrt(len(arr))  # SEM for narrow CI

# Normalized time axis (0-100% of beat cycle)
...
for lead_idx in range(12):
    ...
    # Mark R-peak position (~35% of beat cycle)
...
fig.suptitle("Average single-beat ECG morphology by DL-ECG risk tertile\n"
...
fig.suptitle("Average single-beat morphology by DL-ECG risk group",
...
plot_two_group_comparison(
    ...
    "Average single-beat ECG: Low-risk vs High-risk",
...
plot_two_group_comparison(
    ...
    "Average single-beat ECG: Intermediate-risk vs High-risk",
